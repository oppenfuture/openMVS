...
#define APPNAME _T("RefineMeshBs")
...
bool Initialize(size_t argc, LPCTSTR* argv)
{
	...
	config.add_options()
		...
		("resolution-level", boost::program_options::value<unsigned>(&OPT::nResolutionLevel)->default_value(2), "how many times to scale down the images before mesh refinement")
		...
		("decimate", boost::program_options::value<float>(&OPT::fDecimateMesh)->default_value(0), "decimation factor in range [0..1] to be applied to the input surface before refinement (0 - auto, 1 - disabled)")
...
int main(int argc, LPCTSTR* argv)
{
	...
	TD_TIMER_START();
	if (!scene.RefineMeshBs())
		return EXIT_FAILURE;
	VERBOSE("Mesh refinement using bilateral solver completed: %u vertices, %u faces (%s)", scene.mesh.vertices.GetSize(), scene.mesh.faces.GetSize(), TD_TIMER_GET_FMT().c_str());
